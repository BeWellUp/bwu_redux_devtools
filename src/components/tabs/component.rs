#![allow(
    dead_code,
    reason = "copied official component; kept close to upstream"
)]
...
use dioxus_primitives::{
    ...
    merge_attributes,
    tabs::{self, TabContentProps, TabListProps, TabTriggerProps},
};
...
pub fn TabList(props: TabListProps) -> Element {
    let base = attributes!(div {
        class: format!("tabs {}", Styles::dx_tabs_list)
    });
    let merged = merge_attributes(vec![base, props.attributes]);
